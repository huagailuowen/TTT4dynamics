...
### 1.3 Reward / value function 可能定义了最好的 task-specific latent

或许最自然的潜空间提取方式不是先手工定义 object state，而是通过 reward、value function 或 Q function 来定义 task-specific state。如果我们能够给出一个足够合理的 reward / value objective，并且有足够丰富的数据，那么模型被迫把高维 observation 压缩到一个很小的 latent space，再仅通过这个 latent 去预测 reward、value 或 Q，这个 latent 就会倾向于保留“当前任务成功最需要的信息”。更准确地说，instant reward 和 state 之间并不是完全等价的；真正决定当前 decision 的通常是 value / Q function，因为它包含了当前状态和动作对未来任务结果的影响。因此，计算或估计 `V(s)` / `Q(s, a)` 所需要的最小信息，可能就是我们想要的 effective working state。

这个视角也解释了为什么 working state 会随任务阶段自动变化。比如把物体放到目标位置时，正常情况下 value 主要取决于 object pose 和 goal pose 是否接近；但如果机械臂快要碰到障碍物或桌面，collision avoidance 对未来 value 的影响突然变得更大，于是有效潜空间会自动转向 hand-obstacle clearance、safe direction、collision margin 等变量。平时这些避障变量对 value 几乎没有影响，所以会被压缩掉；一旦它们开始主导任务成败，就会进入 working state。

从认知直觉上说，最初的“本能 reward”可能由生理结构决定；但更高层的智能体可以根据 language goal、task condition 或 imagined outcome 构造 artificial reward / value objective。对应到我们的模型，就是让多模态模型在 goal condition 下把当前高维状态压缩到一个小潜空间，并要求这个潜空间足以预测 value / Q / future task progress。这样学出的 latent 不是通用语义记忆，也不是固定 object-centric state，而是当前任务和当前阶段最影响未来成功率的最小信息集合。
...
2. 人为设计一个简单的 heuristic reward / value / progress function，训练一个 bottleneck hidden-state encoder，让模型只能通过很小的 latent 去预测这个 function；
3. 检查这个 hidden state 是否自动保留 task-stage-relevant variables，例如接近目标时保留 object-goal distance，接近碰撞时保留 hand-obstacle clearance；
4. 再尝试让 model 从 image / depth / proprioception 中学习提取这些 state；
5. 最后探索是否可以用 attention、prediction error、goal conditioning 或 VLM prior 自动发现阶段相关 state。

这个 heuristic function 实验是一个重要的 intermediate verification。它不要求我们一开始就解决“自动发现 reward / value”的完整难题，而是先验证一个更小的问题：

```text
given a task-specific heuristic value/progress target,
can a small latent encoder learn the minimal state needed to predict it?
```

如果可以，说明 reward / value bottleneck 可能确实是提取 working state 的可行路径；如果不行，则说明直接从 image 到 task-specific latent 可能过难，第一版应该继续依赖 human prior / state tracking。
