...
DEMO_SPEED = 0.12
TASK_PROMPT = [
    "dynamic carrier physical grasp probe",
    "observe a moving flat platform, physically grasp the cream cheese with the real gripper, and place it on the target",
    "no fake attach; robosuite fingerpad grasp is required",
    "physical grasp demonstration",
]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--repo-id", default="ttt_dynamic_carrier_physical_grasp_probe")
...
def save_rollout(dataset: LeRobotDataset, *, physical_probe: Any, template: Any, case: Any, seed: int, time_offset_s: float, xy_offset: np.ndarray) -> dict[str, Any]:
    ...
    planner = physical_probe.TimeBasedPhysicalDirectPlanner(
        None,
        env,
    ...
    try:
        ...
        success = bool(final_xy_error <= float(case.target_radius) and grasp_true_frames >= 20 and max_payload_lift >= 0.04)
        if success:
        ...
        return {"success": success, "episode_index": episode_index, "steps": int(frame_count), "final_payload_xyz": final_payload.astype(float).tolist(), "final_xy_error_m": final_xy_error, "max_payload_lift_m": max_payload_lift, "initial_payload_z": initial_payload_z, "robosuite_grasp_true_frames": grasp_true_frames, "phase_counts": phase_counts, "case": case.as_dict(), "trajectory_config": _trajectory_config(template, time_offset_s, xy_offset), "initial_conditions": initial_conditions, "task_description": task_description, "rows": rows}
    finally:
        env.close()
...
def main() -> None:
    ...
    template = {item.name: item for item in piecewise.make_templates()}["line_bounce"]
    ...
    metadata = {"created_at": dt.datetime.now().isoformat(), "dataset_type": "dynamic_carrier_physical_grasp_probe_no_fake_attach", "episodes_requested": int(args.episodes), "seed": int(args.seed), "camera_resolution": CAMERA_RESOLUTION, "fps": FPS, "observe_frames": OBSERVE_FRAMES, "post_done_settle_frames": POST_DONE_SETTLE_FRAMES, "case_id": case.case_id, "template_name": template.name, "physical_grasp_policy": {"fake_attach": False, "robosuite_grasp_required": True, "grasp_target": "payload_position_z + 0.010 m; calibrated from official LIBERO static grasp probe", "physics_mode": "enable physical payload after 10 close-grasp frames", "success_criterion": "final xy within target_radius, robosuite _check_grasp true for >=20 frames, payload lift >=4cm"}, "successes": [], "failures": []}
    metadata_path = output / "dynamic_carrier_physical_grasp_probe_metadata.json"
    successes = 0
    attempts = 0
    while successes < int(args.episodes) and attempts < int(args.max_attempts):
        time_offset_s = float(0.25 * template.period)
        xy_offset = np.zeros(2, dtype=np.float64)
        result = save_rollout(dataset, physical_probe=physical_probe, template=template, case=case, seed=int(args.seed + attempts), time_offset_s=time_offset_s, xy_offset=xy_offset)
        attempts += 1
        if result["success"]:
            successes += 1
            metadata["successes"].append(result)
            print(f"[success] {successes:03d}/{int(args.episodes):03d} attempt={attempts:04d} grasp={result['robosuite_grasp_true_frames']} lift={result['max_payload_lift_m']:.3f} final_xy={result['final_xy_error_m']:.3f} steps={result['steps']}", flush=True)
        else:
            slim = {key: value for key, value in result.items() if key != "rows"}
            metadata["failures"].append(slim)
            print(f"[failed] success={successes:03d}/{int(args.episodes):03d} attempt={attempts:04d} grasp={result['robosuite_grasp_true_frames']} lift={result['max_payload_lift_m']:.3f} final_xy={result['final_xy_error_m']:.3f}", flush=True)
