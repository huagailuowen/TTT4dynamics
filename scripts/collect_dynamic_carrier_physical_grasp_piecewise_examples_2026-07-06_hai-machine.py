...
DEFAULT_OUTPUT = REPO_ROOT / "data" / "lerobot" / "dynamic_carrier_physical_grasp_flat_linebounce_probe_20eps_2026-07-06_hai-machine"
DEFAULT_OUTPUT = REPO_ROOT / "data" / "lerobot" / "dynamic_carrier_physical_grasp_piecewise_examples_14eps_2026-07-06_hai-machine"
...
DEMO_SPEED = 0.12
EXAMPLE_SUCCESS_RADIUS_M = 0.060
TASK_PROMPT = [
    "dynamic carrier physical grasp piecewise examples",
    "observe a moving flat platform, physically grasp the cream cheese with the real gripper, and place it on the target",
    "piecewise trajectory examples with real robosuite fingerpad grasp",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--episodes", type=int, default=len(TEMPLATE_NAMES))
    parser.add_argument("--max-attempts", type=int, default=140)
    parser.add_argument("--seed", type=int, default=20260706)
    parser.add_argument("--overwrite", action="store_true")
    parser.add_argument("--repo-id", default="ttt_dynamic_carrier_physical_grasp_piecewise_examples")
...
def save_rollout(dataset: LeRobotDataset, *, physical_probe: Any, template: Any, case: Any, seed: int, time_offset_s: float, xy_offset: np.ndarray, grasp_xy_offset: np.ndarray, place_xy_compensation: np.ndarray) -> dict[str, Any]:
    ...
    try:
        ...
        success_radius = max(float(case.target_radius), float(EXAMPLE_SUCCESS_RADIUS_M))
...
def main() -> None:
    ...
    plan = build_example_plan(int(args.episodes))
    metadata = {"created_at": dt.datetime.now().isoformat(), "dataset_type": "dynamic_carrier_physical_grasp_piecewise_examples_no_fake_attach_execution", "episodes_requested": int(args.episodes), "seed": int(args.seed), "camera_resolution": CAMERA_RESOLUTION, "fps": FPS, "observe_frames": OBSERVE_FRAMES, "post_done_settle_frames": POST_DONE_SETTLE_FRAMES, "case_id": case.case_id, "template_plan": plan, "phase_candidates": PHASE_CANDIDATES, "grasp_xy_offset_candidates": GRASP_XY_OFFSET_CANDIDATES, "place_xy_compensation_by_template": PLACE_XY_COMPENSATION_BY_TEMPLATE, "example_success_radius_m": EXAMPLE_SUCCESS_RADIUS_M, "physical_grasp_policy": {"fake_attach": False, "robosuite_grasp_required": True, "grasp_target": "payload_position_z + 0.010 m; calibrated from official LIBERO static grasp probe", "intercept_policy": "lock predicted intercept point once per attempt; search small grasp xy offsets for curved trajectories", "placement_policy": "template-specific EEF target compensation is recorded when physical grasp has systematic payload-to-EEF offset", "physics_mode": "enable physical payload after 10 close-grasp frames, lift after 18 grasp-phase frames", "success_criterion": "example/demo threshold: final xy within max(case target_radius, 0.060m), robosuite _check_grasp true for >=20 frames, payload lift >=4cm"}, "successes": [], "failures": []}
    metadata_path = output / "dynamic_carrier_physical_grasp_piecewise_examples_metadata.json"
    successes = 0
    attempts = 0
    per_template_attempts: dict[str, int] = {}
    while successes < int(args.episodes) and attempts < int(args.max_attempts):
        ...
        template_attempt = per_template_attempts.get(template.name, 0)
        phase_fraction = PHASE_CANDIDATES[template_attempt % len(PHASE_CANDIDATES)]
        grasp_offset_index = (template_attempt // len(PHASE_CANDIDATES)) % len(GRASP_XY_OFFSET_CANDIDATES)
        grasp_xy_offset = np.asarray(GRASP_XY_OFFSET_CANDIDATES[grasp_offset_index], dtype=np.float64)
        time_offset_s = float(phase_fraction * template.period)
        xy_offset = np.zeros(2, dtype=np.float64)
        place_xy_compensation = np.asarray(PLACE_XY_COMPENSATION_BY_TEMPLATE.get(template.name, (0.0, 0.0)), dtype=np.float64)
        ...
        attempts += 1
        per_template_attempts[template.name] = template_attempt + 1
        result["example_index"] = int(successes)
        result["trajectory_config"]["template_group"] = str(entry["group"])
        result["trajectory_config"]["phase_fraction"] = float(phase_fraction)
        if result["success"]:
            successes += 1
            metadata["successes"].append(result)
            print(f"[success] {successes:03d}/{int(args.episodes):03d} attempt={attempts:04d} template={template.name} phase={phase_fraction:.2f} grasp_offset={grasp_xy_offset.tolist()} grasp={result['robosuite_grasp_true_frames']} lift={result['max_payload_lift_m']:.3f} final_xy={result['final_xy_error_m']:.3f} steps={result['steps']}", flush=True)
        else:
            slim = {key: value for key, value in result.items() if key != "rows"}
            metadata["failures"].append(slim)
            print(f"[failed] success={successes:03d}/{int(args.episodes):03d} attempt={attempts:04d} template={template.name} phase={phase_fraction:.2f} grasp_offset={grasp_xy_offset.tolist()} grasp={result['robosuite_grasp_true_frames']} lift={result['max_payload_lift_m']:.3f} final_xy={result['final_xy_error_m']:.3f}", flush=True)
