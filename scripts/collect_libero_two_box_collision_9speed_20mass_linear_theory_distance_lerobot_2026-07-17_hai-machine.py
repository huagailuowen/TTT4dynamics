...
DEFAULT_CONFIG = REPO_ROOT / "configs" / "libero_two_box_collision_9speed_20mass_linear_theory_distance_2026-07-17_hai-machine.json"
DEFAULT_OUTPUT = REPO_ROOT / "data/mass/linear_theory_distance_9speed_20mass_2026-07-17_hai-machine"
...
def main() -> None:
    ...
    dataset_root = output_root / "libero_two_box_collision_9speed_20mass_linear_theory_distance_180eps_lerobot_2026-07-17_hai-machine"
    ...
    metadata: dict[str, Any] = {
        ...
        "target_nominal_theory_distances_cm": config["target_nominal_theory_distances_cm"],
        "target_postcollision_vx_mps": config["target_postcollision_vx_mps"],
        "linear_distance_calibration": config["linear_distance_calibration"],
    ...
    for action_cfg in actions_cfg:
        ...
        for mass_index, target_mass_kg in enumerate(masses):
            ...
            row = {
                ...
                "linear_distance_level_short_to_long": int(mass_index),
                "reference_nominal_theory_distance_cm": float(
                    config["target_nominal_theory_distances_cm"][mass_index]
                ),
                "reference_postcollision_vx_mps": float(
                    config["target_postcollision_vx_mps"][mass_index]
                ),
                "projectile_mass_kg": float(config["projectile_mass_kg"]),
                "friction_mu": float(config["friction_mu"]),
